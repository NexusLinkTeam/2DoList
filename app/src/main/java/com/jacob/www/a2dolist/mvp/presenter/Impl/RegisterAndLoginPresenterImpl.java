...
public class RegisterAndLoginPresenterImpl extends BasePresenterImpl<RegisterAndLoginView> implements RegisterAndLoginPresenter {
    ...
    public void logIn(final LogInActivity context, String uId, String password) {
        ...
        if (TextUtils.isEmpty(password)) {
            Toast.makeText(BaseApp.getAppContext(),"密码不能为空",Toast.LENGTH_SHORT).show();
            context.loginPassWord.setShakeAnimation();
            return;
        }
        if (uId.length() < 4 || uId.length() > 128) {
            context.loginUserName.setShakeAnimation();
            Toast.makeText(BaseApp.getAppContext(),"用户名为4-128位字符",Toast.LENGTH_SHORT).show();
            return;
        }
        if (password.length() < 4 || password.length() > 128) {
            context.loginUserName.setShakeAnimation();
            Toast.makeText(BaseApp.getAppContext(),"密码为4-128位字符",Toast.LENGTH_SHORT).show();
            return;
        }
        if (isContainChinese(uId)) {
            context.loginUserName.setShakeAnimation();
            Toast.makeText(BaseApp.getAppContext(),"用户名不支持中文",Toast.LENGTH_SHORT).show();
            return;
        }
        if (!whatStartWith(uId)) {
            context.loginUserName.setShakeAnimation();
            Toast.makeText(BaseApp.getAppContext(),"用户名以字母或者数字开头",Toast.LENGTH_SHORT).show();
            return;
        }
        if (!whatContain(uId)) {
            context.loginUserName.setShakeAnimation();
            Toast.makeText(BaseApp.getAppContext(),"只能含有: 数字 字母 下划线 . - @",Toast.LENGTH_SHORT).show();
            return;
        }
        //登录
            ...
            JMessageClient.login(uId, password, new BasicCallback() {
                ...
                public void gotResult(int i, String s) {
                    ...
                    if(i ==0){
                        ...
                        Toast.makeText(BaseApp.getAppContext(),"登录成功",Toast.LENGTH_SHORT).show();
                        context.startActivity(new Intent(context, MainActivity.class));
                        context.finish();
                    }else {

                        Toast.makeText(BaseApp.getAppContext(),"登录失败",Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }else {
